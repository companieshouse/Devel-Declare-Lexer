...
lexer_test "Test hashref array access %s", $h->{test}->[0];
++$tests && is($lexed, q/lexer_test "Test hashref array access %s", $h->{test}->[0];/, 'Hashref with arrayref');
...
TODO: {
    ...
    ++$tests && is(__LINE__, 61, 'Line numbering (CHECK WHICH LINE THIS IS ON)');
}
